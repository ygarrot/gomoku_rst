...
pub trait Rule {
    ...
    fn free_threes(&self, board: &mut Board, move_: &Move, player:&Player) -> bool;
}
...
impl Rule for BaseRule {
    ...
    fn free_threes(&self, board: &mut Board, move_: &Move, player:&Player) -> bool{true}
    fn valid(&self, b: &Board, m: &Move) -> bool {
...
impl Rule for FreeThrees{
    ...
    fn free_threes(&self, board: &mut Board, move_: &Move, player:&Player) -> bool {
        fn count_dir(player_x: i64, player_y: i64, increment_x: i64,
            increment_y: i64, id: u8, board: &mut Board) -> u8{
               ...
               let y_extremity =  (player_y - increment_y * 2) as usize;
                ...
                let vals = if board.get_fcoo(player_x as usize, player_y as usize) == id {[id, id, 0, 42]} else {[0, id, id, 0]};
    ...
    fn r#type(&self) -> RuleType { RuleType::FREE_THREES }
}
